...
支持任意容器可读路径；相对路径默认按 /workspace 解析。""".trimIndent(),
            ...
            execute = { args ->
                val rawPath = args.jsonObject["path"]?.jsonPrimitive?.contentOrNull
                    ?.trim()
                    ...
                    ?: return@Tool listOf(
                        UIMessagePart.Text(
                            buildJsonObject {
                                put("success", JsonPrimitive(false))
                                put("error", JsonPrimitive("Missing required parameter: path"))
                            }.toString()
                        )
                    )
                        ...
                        UIMessagePart.Text(
                            buildJsonObject {
                                ...
                                put(
                                    "error",
                                    JsonPrimitive(
                                        "Unsupported image extension '.$extension'. Supported extensions: ${supportedExtensions.joinToString(", ")}"
                                    )
                                )
                            }.toString()
                        )
            ...
            when (option) {
                ...
                LocalToolOption.Container -> {
                    if (prootManager.isRunning) {
                        "Container Tools (4): container_shell, container_shell_bg, container_process, container_view_image"
                    } else {
                        null // 容器未运行时不显示
                    }
                }
